      ...
      >
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4">
          <div>
            <nav className="flex text-xs text-slate-400 gap-2 mb-2">
              ...
              <span>/</span>
              <span className="text-green-800 font-medium">Reportes de Deducción</span>
            </nav>
            <h2
          ...
          <div className="flex items-center justify-center gap-3 bg-slate-100 px-4 py-2 rounded-2xl shrink-0">
            <span className="material-symbols-outlined text-green-800">calendar_month</span>
            <span className="text-sm font-semibold text-gray-900">Octubre 2023 - Noviembre 2023</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {KPIS.map((kpi) => (
            ...
            >
              <p className="text-sm font-medium text-slate-500 group-hover:text-white/80 transition-colors">{kpi.label}</p>
          ...
          >
            <div className="relative z-10 w-full">
              <p className="text-sm font-medium text-green-200">Eficiencia de Recuperación</p>
              <div className="flex items-end justify-between mt-2">
                <h3
          ...
          <div className="bg-white rounded-[2rem] overflow-hidden shadow-sm border border-slate-100">
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse min-w-[800px]">
                ...
                <tbody className="divide-y divide-slate-50">
                    ...
                    <tr key={row.id} className="group hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-bold text-gray-900 leading-tight" style={{ fontFamily: "Manrope, sans-serif" }}>
                              {row.name}
                            </p>
                      ...
                      {/* Salary */}
                      <td className="px-6 py-5 text-center text-sm font-medium text-slate-500">{row.salary}</td>

                      {/* Credit */}
                      <td className="px-6 py-5 text-right font-bold text-gray-900" style={{ fontFamily: "Manrope, sans-serif" }}>
                        {row.credit}
                      </td>

                      {/* Quota */}
                      <td className="px-6 py-5 text-center">
                        <span className="text-sm font-semibold bg-slate-100 px-3 py-1 rounded-full">{row.quota}</span>
                      </td>

                      {/* Amount */}
                      <td className="px-6 py-5 text-right font-bold text-green-800" style={{ fontFamily: "Manrope, sans-serif" }}>
                        {row.amount}
                      </td>

                      {/* Date */}
                      <td className="px-6 py-5 text-sm text-slate-400">{row.date}</td>
                  ...
                  {filtered.length === 0 && (
                    <tr>
                      <td colSpan={7} className="px-8 py-16 text-center text-slate-400 font-bold italic text-sm">
            ...
            <div className="p-6 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
              <p className="text-sm text-slate-400">
                Mostrando <span className="font-bold text-gray-900">{filtered.length}</span> de 128 registros
              </p>
                ...
                {[1, 2].map((n) => (
                  <button
                    ...
                    className={`w-10 h-10 flex items-center justify-center rounded-xl font-bold text-sm transition-colors ${
                      n === 1 ? "bg-green-800 text-white" : "bg-white border border-slate-200 hover:bg-green-50 text-gray-600"
                    }`}
                  >
                    {n}
                  </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-12">
          ...
          <div className="lg:col-span-2 bg-slate-100 rounded-3xl p-8 flex items-center gap-8 relative overflow-hidden flex-col sm:flex-row">
            ...
            <div className="relative z-10 w-full sm:w-auto">
              ...
              <p className="text-sm text-slate-500 mb-6 max-w-md">
                Basado en la política de deducción del 15% para el próximo cierre de nómina.
              </p>
              <div className="flex gap-6">
                <div className="flex flex-col">
                  <span className="text-[10px] uppercase font-black text-slate-400 tracking-widest">Recuperación Estimada</span>
                  <span
                ...
                <div className="flex flex-col">
                  <span className="text-[10px] uppercase font-black text-slate-400 tracking-widest">Balance Pendiente</span>
                  <span
          ...
          >
            <div>
              <span className="material-symbols-outlined text-4xl mb-4 block opacity-50">verified_user</span>
              <h4
              ...
              <p className="text-sm text-green-200/80 mt-2 leading-relaxed">
                Deducciones automáticas configuradas al 15% del salario mensual para todos los créditos activos.
              </p>
            </div>
