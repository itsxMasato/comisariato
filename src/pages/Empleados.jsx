                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div className="col-span-1 md:col-span-2 bg-slate-100 p-8 rounded-3xl relative overflow-hidden group">
                        <div className="relative z-10">
                            ...
                            <h3 className="text-4xl font-extrabold text-green-900 mt-4 tracking-tighter">{fmtL(428500)}</h3>
                    ...
                    <div className="p-8 rounded-3xl text-white flex flex-col justify-between bg-gradient-to-br from-[#6c493d] to-[#523327]">
                        ...
                        <p className="text-2xl font-extrabold">{fmtL(1200000)}</p>
                    </div>
                </div>
                    ...
                    <table className="w-full border-collapse text-left">
                        <thead>
                            <tr className="bg-slate-50 border-b border-slate-100">
                                {["Empleado", "DNI / Código", "Crédito Asignado", "Saldo Actual", "Acciones"].map((h) => (
                        ...
                        <tbody className="divide-y divide-slate-50 text-sm">
                            {filtered.map((emp) => (
                                <tr key={emp.empleadoId} className="group hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className={`px-8 py-5 font-bold ${emp.estado === 'limit' ? 'text-red-600' : 'text-gray-900'}`}>{fmtL(emp.balance || 0)}</td>
                                    <td className="px-8 py-5">
                        ...
                        <div className="p-10 space-y-6">
                            ...
                            <form className="grid grid-cols-1 md:grid-cols-2 gap-5" onSubmit={handleSave}>
                                ...
                                <div className="space-y-1">
                                    ...
                                    <input type="number" required value={formData.limiteCredito} onChange={e => setFormData({ ...formData, limiteCredito: parseFloat(e.target.value) })} className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-5 py-2.5 text-sm outline-none focus:border-green-600" />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black uppercase text-slate-400 ml-1">Salario (L)</label>
                                    <input type="number" required value={formData.salario} onChange={e => setFormData({ ...formData, salario: parseFloat(e.target.value) })} className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-5 py-2.5 text-sm outline-none focus:border-green-600" />
                                </div>
