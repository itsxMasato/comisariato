...
export default function Empleados() {
    ...
    useEffect(() => {
        const unsubHistorial = onSnapshot(
            collection(db, "historial_empleados"),
            (snapshot) => {
                setHistorialData(
    ...
    const employees = employeesData.map((e) => ({
        ...
        telefono: e.telefono || "",
        fechaRegistro:
            e.fechaRegistro && typeof e.fechaRegistro.toDate === "function"
                ? e.fechaRegistro.toDate().toLocaleDateString()
                : e.fechaRegistro || "",
        balance: 0,
        img: `https://ui-avatars.com/api/?name=${e.nombres ? e.nombres[0] : "U"}`,
    }));

    const totalActivos = employees.filter(
        (e) => e.estado === "active" || e.estado === "Activo",
    ).length;
    const totalInactivos = employees.filter(
        (e) => e.estado === "inactive" || e.estado === "Inactivo",
    ).length;
    const sumaSalarios = employees.reduce((acc, curr) => acc + curr.salario, 0);

    const inactivosEnNomina = employees.filter(
        (e) => e.estado === "inactive" || e.estado === "Inactivo",
    ).length;
    ...
    historialData
        .filter(
            (h) =>
                h.tipoModificacion === "Eliminación de Empleado" &&
                !employeesData.find((e) => e.id === h.empleadoId),
        )
        .sort(
            (a, b) =>
                (b.fechaModificacion?.seconds || 0) -
                (a.fechaModificacion?.seconds || 0),
        )
        .forEach((h) => {
    ...
    const handleOpenModal = (emp = null) => {
        if (emp) {
            ...
            // Calcular siguiente código EMP- automáticamente
            let nextId = 1;
            if (employeesData.length > 0) {
                employeesData.forEach((e) => {
                    if (e.codigoEmpleado && e.codigoEmpleado.startsWith("EMP-")) {
                        const num = parseInt(e.codigoEmpleado.replace("EMP-", ""), 10);
            ...
            const nuevoCodigo = `EMP-${nextId.toString().padStart(4, "0")}`;
    ...
    const handleSave = async (e) => {
        ...
        const payload = {
            ...
            tipoModificacion: editingEmployee
        ...
        try {
            if (editingEmployee) {
                await updateDoc(
                    doc(db, "empleados", editingEmployee.empleadoId),
                    payload,
                );
            } else {
                ...
                // --- Creación Automática de Usuario (Portal/App) ---
                if (payload.correo) {
                    try {
                        // Extraemos el primer nombre (en minúsculas y sin acentos ni espacios)
                        const primerNombre = payload.nombres
                            .trim()
                            .split(" ")[0]
                            .toLowerCase()
                            .normalize("NFD")
                            .replace(/[\u0300-\u036f]/g, "")
                            .replace(/[^a-z0-9]/g, "");
                        const systemEmail = `${primerNombre}@comisariato.pro`;
                        const generatedPass = `${primerNombre}2026`;
                        ...
                        await setDoc(doc(db, "usuarios", cred.user.uid), {
                            ...
                            uid: cred.user.uid,
                            empleadoId: docRef.id,
                            nombre: `${payload.nombres} ${payload.apellidos}`,
                            correo: systemEmail, // Correo institucional para LOGUEARSE
                            correoPersonal: payload.correo, // Correo para contacto/recuperación
                            rol: "EMPLEADO", // Automático para App Móvil
                            estado: "Activo",
                            tipoModificacion: "Alta Automática desde Empleados",
                            usuarioModifico: auth.currentUser?.email || "Admin",
                        ...
                        await updateDoc(docRef, {
                            uid: cred.user.uid,
                            usuarioId: cred.user.uid,
                            correoInstitucional: systemEmail,
                        });
                        ...
                        if (
                            ...
                            try {
                                await emailjs.send(
                                    ...
                                    TEMPLATE_ID,
                                    {
                                        to_email: payload.correo, // <-- Correo a donde llega el mensaje
                                        user_name: payload.nombres,
                                        user_role: "EMPLEADO",
                                        system_email: systemEmail, // <-- El correo que usará para entrar
                                        temp_password: generatedPass,
                                    },
                                    PUBLIC_KEY,
                                );
                                console.log("Correo automático de bienvenida enviado");
                            } catch (e) {
                                console.error("Error al enviar emailJS:", e);
                                alert(
                                    "El empleado y usuario fueron creados EXITOSAMENTE, pero hubo un fallo enviando el correo de bienvenida. Revisa la consola y tus llaves de EmailJS.",
                                );
                            }
                        } else {
                            console.warn(
                                "EmailJS no está totalmente configurado. Falta el TEMPLATE_ID. El correo no fue enviado.",
                            );
                        }
                    } catch (errUser) {
                        console.error("Error creando usuario auto:", errUser);
                        const msg =
                            errUser.message ||
                            (errUser.text ? errUser.text : JSON.stringify(errUser));
                        alert(
                            "Se registró el empleado pero hubo un error generando su usuario de App: " +
                            msg,
                        );
                    }
                }
    ...
    const handleExportReport = (deptReport = "Todos los Departamentos") => {
        const listToExport =
            deptReport === "Todos los Departamentos"
                ? filtered
        ...
        const totalSalary = listToExport.reduce(
            (acc, curr) => acc + (curr.salario || 0),
            0,
        );
        const totalCredit = listToExport.reduce(
            (acc, curr) => acc + (curr.limiteCredito || 0),
            0,
        );
            ...
            >
                ...
                <div className="flex justify-between items-end gap-4 flex-wrap">
                    <div>
                        ...
                        <p className="text-slate-500 font-medium">
                            Gestion de colaboradores, salarios y credito automatico por
                            empleado.
                        </p>
                    </div>
                    ...
                    <div className="flex items-center gap-3 w-full sm:w-auto">
                        <div className="relative group text-sm font-bold w-full sm:w-auto z-40 flex-1 sm:flex-none">
                            ...
                            <div className="absolute right-0 top-full mt-2 w-56 bg-white border border-slate-200 rounded-2xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all flex flex-col p-2">
                                ...
                                <button
                                    onClick={() => handleExportReport("Todos los Departamentos")}
                                ...
                                {departamentos.map((depto) => (
                                    <button
                                        key={depto.id}
                                        onClick={() => handleExportReport(depto.nombre)}
                                        className="px-4 py-2 hover:bg-slate-50 text-left rounded-xl text-slate-700 text-xs font-bold transition-all"
                                    >
                                        {depto.nombre}
                                    </button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    ...
                    <div className="col-span-1 md:col-span-2 bg-slate-100 p-8 rounded-3xl relative overflow-hidden group">
                        <div className="relative z-10">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                                Inversión Mensual en Salarios
                            </span>
                            <h3
                                className="text-4xl font-extrabold text-green-900 mt-4 tracking-tighter"
                                style={{ fontFamily: "Manrope, sans-serif" }}
                            >
                                {fmtL(sumaSalarios)}
                            </h3>
                    ...
                    <div className="p-8 rounded-3xl text-white flex flex-col justify-between bg-gradient-to-br from-[#6c493d] to-[#523327]">
                        <span className="text-[10px] font-black text-amber-100/50 uppercase tracking-widest">
                            Personal Inactivo
                        </span>
                        <div>
                            <p
                            ...
                            <p className="text-[10px] text-amber-100/40 font-medium">
                                Bajas y registros archivados
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-3xl overflow-hidden shadow-sm border border-slate-100 flex flex-col">
                    <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
                        <h5 className="text-sm font-bold text-slate-800 uppercase tracking-widest hidden sm:block">
                            Planilla
                        </h5>
                        ...
                        <thead>
                            <tr className="bg-slate-50 border-b border-slate-100">
                                {[
                                    ...
                                    <th
                                        key={h}
                                        className="px-8 py-5 text-xs font-black text-slate-400 uppercase tracking-widest"
                                    >
                                        {h}
                                    </th>
                                ))}
                                ...
                                >
                                    ...
                                    <td className="px-8 py-5">
                                        ...
                                        <p className="text-[10px] text-slate-400 uppercase">
                                            {emp.codigoEmpleado} • {emp.departamento}
                                        </p>
                                    </td>
                                    <td className="px-8 py-5 font-bold text-green-800">
                                        {fmtL(emp.limiteCredito)}
                                    </td>
                                    <td
                                        className={`px-8 py-5 font-bold ${emp.estado === "limit" ? "text-red-600" : "text-gray-900"}`}
                                    >
                                        {fmtL(emp.salario)}
                                    </td>
                                    <td className="px-8 py-5">
                            ...
                            >
                                ...
                                <div className="space-y-1">
                                    ...
                                    <input
                                        ...
                                        maxLength={15}
                                        pattern=".{15}"
                                        title="El DNI debe tener los 13 dígitos completos."
                                        value={formData.dni}
                                        onChange={(e) => {
                                ...
                                <div className="space-y-1">
                                    ...
                                    <input
                                        ...
                                        title="Por favor ingrese un correo electrónico válido (ej: nombre@dominio.com)."
                                ...
                                <div className="space-y-1">
                                    ...
                                    <input
                                        ...
                                        value={
                                            formData.salario === 0
                                                ? ""
                                                : formData.salario.toLocaleString("en-US")
                                        }
                                        onChange={(e) => {
                                            ...
                                            const nuevoLimite =
                                                nuevoSalario * (parametros.porcentajeSueldo / 100);
                                            setFormData({
                                                ...formData,
                                ...
                                <button
                                    type="submit"
                                    className="md:col-span-2 w-full bg-green-900 text-white py-4 rounded-2xl font-black uppercase tracking-widest text-xs shadow-lg hover:bg-green-800 transition-all mt-4 active:scale-[0.98]"
                                >
                                    {editingEmployee
                                        ? "Actualizar Empleado"
                                        : "Finalizar Registro"}
                    ...
                    <div className="bg-white w-full max-w-xl rounded-[2.5rem] shadow-2xl relative p-10 space-y-8 animate-in fade-in zoom-in duration-200">
                        ...
                        <div className="grid grid-cols-2 gap-y-6 gap-x-10 text-sm">
                            {[
                                ...
                                {
                                    label: "Salario Base",
                                    value: fmtL(selectedEmployee.salario),
                                },
                                {
                                    label: "Límite de Crédito",
                                    value: fmtL(selectedEmployee.limiteCredito),
                                },
                                {
                                    label: "Saldo Actual",
                                    value: fmtL(selectedEmployee.balance || 0),
                                },
                                {
                                    label: "Estado",
                                    value:
                                        selectedEmployee.estado === "active"
                                            ? "ACTIVO"
                                            : "INACTIVO",
                                },
                            ].map((item) => (
